...
import ResultCard from "../Patient/ResultCard";

const diseases = [
  { key: "brain_tumor", label: "Brain Tumor", icon: <Brain size={20} /> },
  { key: "diabetic_retinopathy", label: "Diabetic Retinopathy", icon: <Eye size={20} /> },
  { key: "skin_cancer", label: "Skin Cancer", icon: <Scan size={20} /> },
];
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-5">Analyze Patient Scan</h2>
          ...
          {diseases.map((d) => (
            <button
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 text-sm font-medium transition
                ${selectedDisease === d.key ? "border-blue-600 bg-blue-50 text-blue-600" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
            >
              {d.icon}{d.label}
            </button>
          ))}
          ...
          {preview
            ...
            : (
              <div className="flex flex-col items-center gap-3 text-slate-400">
                <Upload size={36} />
                <p className="text-sm">Drag & drop scan or click to browse</p>
              </div>
            )}
        </div>
        <button
